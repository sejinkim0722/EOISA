		...
		<div class="sidenav component-mobile">
			...
			<sec:authorize access="isAuthenticated()">
				<div class="sidenav-profile">
					...
					<h4 class="text-dark">
						<strong>${userinfo.nickname}</strong> <small class="text-black-50">${userinfo.platform}</small>
					</h4>
				</div>
			</sec:authorize>
			<hr>
					...
					<div id="userinfo" class="sidebar-box component-desktop">
						<div id="userinfo-header">
							...
							<h5 class="text-dark">
								...
								<small class="text-black-50">${userinfo.platform}</small>
							</h5>
						</div>
						<hr>
			...
			<div id="main-content-wrapper" class="col-lg-6 col-md-12">
				...
				<div id="main-content">
								...
								<div class="post-group"<c:if test="${not empty deal.isended}">style="opacity: 0.25;"</c:if>>
									<div class="deal-header">
										<p>
											<span>
											<c:choose>
												<c:when test="${empty deal.isended}">
													<label class="bg-secondary"><i class="fas fa-stopwatch"></i>${deal.writetime}</label>
												</c:when>
												<c:otherwise>
													<label class="bg-dark"><i class="fas fa-stopwatch"></i><strong>${deal.isended}</strong></label>
												</c:otherwise>
											</c:choose> 
											<c:if test="${not empty deal.site_buy}">
												<label class="bg-info"><i class="fas fa-shopping-bag"></i>${deal.site_buy}</label>
											</c:if> 
											<c:if test="${not empty deal.site_src}">
									...
									<div class="thumbnail">
										<c:choose>
											<c:when test="${empty deal.goods_pic}">
												<img src="/resources/assets/dealimg.png" alt="deal_thumbnail_default">
											</c:when>
											<c:otherwise>
												<img src="${deal.goods_pic}" alt="deal_thumbnail">
											</c:otherwise>
										</c:choose>
									</div>
									<div class="dealinfo">
									...
									<div class="opinion">
										<p>
											<c:set var="i" value="far fa-heart" />
											<c:forEach var="wishlist" items="${wishlist}">
												<c:if test="${wishlist.dealno eq deal.dealno}">
													<c:set var="i" value="far fa-heart fas" />
												</c:if>
											</c:forEach>
											<button type="button" class="btn btn-xs btn-wish" data-dealno="${deal.dealno}" <sec:authorize access='!isAuthenticated()'>data-toggle="modal" data-target="#modal-signin"</sec:authorize>>
												<i class="${i}"></i> 찜하기
											</button>
											<button type="button" class="btn btn-xs btn-share" data-clipboard-text="https://eoisa.ml/deal/${deal.dealno}">
												<i class="fas fa-share-alt"></i> 공유하기
											</button>
											<c:if test="${deal.replycount ne 0}">
												<button type="button" class="btn btn-xs btn-toggle" data-dealno="${deal.dealno}">
					...
					<div class="ad-banner bottom">
						<img src="/resources/assets/adbanner.png" alt="ad_banner">
					</div>
			...
			<div id="right-sidebar" class="col-md-3 component-desktop">
				<div id="ranking" class="sidebar-box">
					<div id="ranking-header">
						<h5 class="text-dark">
							<i class="fab fa-hotjar"></i> HotDeal Ranking
						</h5>
					</div>
					<hr>
					<div id="ranking-body">
