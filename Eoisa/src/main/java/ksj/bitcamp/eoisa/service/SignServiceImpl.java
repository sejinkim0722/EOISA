...
public class SignServiceImpl implements SignService
{
	...
	@Override
	public int modifyService(SignDTO dto) {
		return dao.modify(dto);
	}
	
	@Transactional
	@Override
	public void emailAuthService(String username, String uuid) {
		dao.emailAuth(username, uuid);
		...
		try {
			...
			sendMail.setText(new StringBuffer()
					...
					.append("<div style='border: 1px solid #ced1cc; padding: 10px;'><h3><font color='#565a5c'>해당 <a href='https://eoisa.ml/verification/" + username + "/" + uuid + "' target='_blank' style='text-decoration: none;'>링크</a>를 클릭하시면 인증 절차가 완료됩니다.</font></h3></div>")
	...
	@Override
	public int verificationService(String username, String uuid) {
		return dao.verification(username, uuid);
	}
		...
		if(dao.findPassword(username).equals("notexist")) {
			...
			try {
				...
				sendMail.setText(new StringBuffer()
						...
						.append("<hr style='margin-top: 30px; margin-bottom: 50px;'>")
						.append("<div style='border: 1px solid #ced1cc; padding: 10px;'><h3><font color='#007a87'>" + dao.findPassword(username) + "</font></h3></div>")
						.append("<br>")
